...
def send_to_discord(item):
    """Отправка сообщения: текст с флагом сверху, разделитель и ссылка в поле снизу"""
    payload = {
        "embeds": [{
            "description": item["desc"],  # Флаг и текст источника строго НАВЕРХУ
            "color": 16744192,            # Оранжевая полоска слева
            "fields": [
                {
                    "name": "\u200b",     # Невидимый заголовок создает ту самую линию-разделитель
                    "value": f"[{item['title']}]({item['link']})",  # Синяя кликабельная ссылка ВНИЗУ
                    "inline": False
                }
            ]
        }]
